...
//            String value = Double.toString(Math.round(x*100.0)/100.0);
        ...
        for(List<Point> graphPointList: graphPoints) {
            ...
            for (int i = 0; i < graphPointList.size() - 1; i++) {
                ...
                if(i%(graphPointList.size()/10)==0) {
                    String value = Double.toString(Math.round(scores.get(graphPoints.indexOf(graphPointList)).get(i)*100.0)/100.0);
